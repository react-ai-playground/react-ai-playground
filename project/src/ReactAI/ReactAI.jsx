...
const ReactAI = () => {
  ...
  const [gitIMGs, setGitIMGs] = useState([
    // Navbars
    [
      {name: "Navbar1", component: "Navbars", img: imgLocation + "Navbar1.png"},
    ...
    // Displays
    [
      {name: "Display1", component: "Displays", img: imgLocation + "Display1.png"},
      {name: "Display2", component: "Displays", img: imgLocation + "Display2.png"},
  ...
  const renderElementTreeNode = (node, path) => {
    if (typeof node !== 'object' || node === true) {
      ...
      return <div className="tree-item" onClick={()=>{
        // handleFileOpen(path);
        console.log("opening")
      }} style={{color: "white", marginBottom: "3px", borderBottom: "0.3px solid #888", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", fontFamily: "sans-serif"}} key={path}>{path.split('/').pop()}</div>; 
    }
    ...
    return (
      <div key={path}>
        <div className="tree-item" style={{overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", borderBottom: "0.3px solid #888", marginBottom: "3px", fontFamily: "sans-serif"}}  onClick={() => handleToggle(path)}>
  ...
  async function handleCreateElementTree() {
    // Parse the current component
    let componentName = "Navbar1"
    let parsedComponent = parseChildren(fakeComponent)
    console.log(parsedComponent)

    // Now structure it 
    let object = {"Navbar1": {"nav": true, "nav*": true}}
    // object[componentName] = {}
    // object[componentName] = Object.assign(object["div"], 1)
    // console.log(object)
    
    let parsedElementTree = structureElementTree(parsedComponent)
    let elements = await retrieveFileTree()
    console.log(elements)

    // Set the tree
    setElementTree(object)
  }

  function structureElementTree(array) {
                    ...
                    <div className="select-none" > 
                      <div className="select-none" style={{userSelect: "none", paddingBottom: "50px", marginTop: "13px"}}>
                        {updateStackNum > 0 && 
                          <>
                            <DraggableList/>
                            <div className="hover-dim" onClick={()=>{newComponent()}} style={{marginTop: "10px", marginLeft: "10px", cursor: "pointer", width: "calc(100% - 10px)", height: "33px", backgroundColor: "black", border: "1px solid white", borderRadius: "7px", display: "flex", justifyContent: "center", alignItems: "center"}}>
                              <HiPlusSm style={{marginLeft: "-10px", marginTop: "-3px"}} color="white" fontSize={26} />
                              <p style={{marginLeft: "-3px", marginTop: "-2px", color: "white", fontWeight: "bold", fontFamily: "sans-serif", fontSize: "14px"}}>
                                Component
                              </p>
                ...
                <div style={{zIndex: 3,  height: "120px", width: "100%", borderTop: "0.1px solid white", borderBottom: "0.1px solid white", backgroundColor: "black", display: "flex", flexDirection: "row", overflow: "scroll"}}>
                  {componentData[componentData.length - 1 - index][1] && componentData[componentData.length - 1 - index][1].map((item, index2) => (
                    <div 
                      ...
                      <img style={{width: "100%", maxHeight: "80px", objectFit: "cover", borderRadius: "5px", border: "1px solid white"}} src={gitIMGs[index][index2].img} alt=""/>
                  </div>
                  ))}
                </div>
              </>
